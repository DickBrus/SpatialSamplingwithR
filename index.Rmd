...
A gitbook version of this book (html files) is available in the github repository [SpatialSamplingwithR]^( https://git.wageningenur.nl/brus003/spatialsamplingwithr/tree/master). Here you can also find the **R** scripts of the exercises. This is also the place where you can report errata and comment on text and/or **R** code. I will update my book now and then, for instance to replace **R** code using package **sp** with code using package **sf**, and to make better use of package **tidyverse**.

<!-- Mention **R** package **sswr** with the data files used in this book. When package **sswr** is not available on CRAN. It must be installed with remotes::github(...) -->
