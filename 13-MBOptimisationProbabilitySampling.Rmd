...
saveRDS(v_mzsim_SY,file = "results/MBpredVariance_SY_Amhara.rds")
```

```{r, echo = FALSE}
v_mzsim_SY <- readRDS(file = "results/MBpredVariance_SY_Amhara.rds")
```
...
```{r, eval = FALSE, echo = FALSE}
saveRDS(MCMCsample, file = "results/MCMC_Melles.rds")
```
...
MCMCsample <- readRDS(file = "results/MCMC_Melles.rds")
d <- 1:150
...
```{r, eval = FALSE, echo = FALSE}
saveRDS(V, file = "results/SimulatedVariances_Melle.rds")
```
...
V <- readRDS(file = "results/SimulatedVariances_Melle.rds")
ggplot() +
...
(ref:predictedlnSOMlabel) Predicted natural log of the SOM concentration (g kg^-1^) in the topsoil of Xuancheng and its standard error (se), obtained with a linear regression model with elevation as a predictor.
...
```{r, eval = FALSE, echo = FALSE}
saveRDS(res, file = "results/OptimalStrata_Xuancheng.rds")
```

```{r, echo = FALSE}
res <- readRDS(file = "results/OptimalStrata_Xuancheng.rds")
```
