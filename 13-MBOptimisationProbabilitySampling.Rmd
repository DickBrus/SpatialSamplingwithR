...
(ref:VariogramLeestlabel) Sample semivariogram and fitted spherical model for NO~3~-N in field Leest. Numbers are numbers of point pairs used in computing semivariances.

```{r VariogramLeest, fig.asp=0.7, fig.cap="(ref:VariogramLeestlabel)"}
library(gstat)
...
```{r qqplotMelle, fig.asp=0.7, fig.cap="(ref:qqplotMellelabel)"}
...
```{r MCMCsampleVariogramlnN, echo=FALSE, fig.asp=0.7, fig.cap="(ref:MCMCsampleVariogramlnNlabel)"}
d <- 1:150
...
(ref:histogramNMellelabel) Histogram of simulated sampling variances of the $\pi$ estimator of the mean of NO~3~-N of field Melle, for stratified simple random sampling, using 25 compact geostrata of equal size, and one point per stratum.

```{r histogramNMelle, echo=FALSE, fig.asp=0.7, fig.cap="(ref:histogramNMellelabel)"}
ggplot() +
...
(ref:predictedlnSOMlabel) Predicted natural log of the SOM concentration (g kg^-1^) in the topsoil of Xuancheng and its standard error, obtained with a linear regression model with elevation as predictor.
