...
library(gstat)
mydata <- read.csv("data/Sample_Leest.csv")
coordinates(mydata) <- ~ s1 + s2
...
The next code chunk shows the computation of the mean semivariance per stratum and the model-based prediction of the sampling variance of the estimator of the mean.
...
The model-based prediction of the sampling variance of the estimator of the mean with this design equals `r as.character(round(Exi_V_SY,1))` (kg ha^-1^)^2^, which is smaller than that of stratified simple random sampling. This can be explained by the improved geographical spreading of the sampling points with systematic random sampling as compared to stratified simple random sampling with compact geographical strata.
...
Package **gstat** is used for geostatistical simulation, and therefore first the REML estimates of the semivariogram parameters are passed to arguments `nugget`, `psill`, and `range` of function `vgm` of this package. 
...
The mean of the 100 conditional variances equals `r round(Exi_vmz_SY,3)` (dag kg^-1^)^2^. This is a Monte Carlo approximation of the model-based prediction of the sampling variance of the ratio estimator of the mean for systematic random sampling with an expected sample size of 50.
...
The optimal number of PSU draws is `r ceiling(nopt)`, and the optimal number of points per PSU draw equals `r ceiling(mopt)`. The total number of sampling points is `r ceiling(nopt)` $\times$ `r ceiling(mopt)` `r ceiling(nopt)*ceiling(mopt)`. This is much larger than the sample size of 50 obtained with systematic random sampling. The total observation costs therefore are substantially larger. However, the access time can be substantially smaller due to the spatial clustering of sampling points. To answer the question whether the costs saved by this reduced access time outweigh the extra costs of observation, the model for the access costs and observation costs must be further developed.
...
mydata <- read.csv("data/Sample_Melle.csv", header = TRUE)
...
```{r, echo=FALSE}
min_s1 <- min(mydata$s1)
min_s2 <- min(mydata$s2)
mydata$s1 <- mydata$s1 - min_s1
mydata$s2 <- mydata$s2 - min_s2
```
...
D <- as.matrix(dist(mydata[, c("s1", "s2")]))
...
```{r, eval = FALSE, echo = FALSE}
save(MCMCsample, file = "results/MCMC_Melles.RData")
```
...
load(file = "results/MCMC_Melles.RData")
d <- 1:150
...
```{r, echo = FALSE}
shpField <- readOGR(dsn = "data", layer = "Melle", verbose = FALSE)
proj4string(shpField) <- NA_character_
mygrid <- spsample(shpField, type = "regular", n = 2000, offset = c(0.5, 0.5)) %>% as(., "data.frame")
mygrid$x1 <- mygrid$x1 - min_s1
mygrid$x2 <- mygrid$x2 - min_s2
gridded(mygrid) <- ~ x1 + x2
n <- 25
mygeostrata <- stratify(mygrid, nStrata = n, equalArea = TRUE, nTry = 10)
```
...
```{r, eval = FALSE, echo = FALSE}
save(V, file = "results/SimulatedSamplingVariances_Melle.RData")
```
...
save(Zsim, v, file = "results/SimulatedMaps_Melle.RData")
```
...
load(file = "results/SimulatedMaps_Melle.RData")
ggplot(data = Zsim) +
...
The sampling variance shows quite strong variation among the maps. The histogram of Figure \@ref(fig:histogramNMelle) shows our uncertainty about the sampling variance, due to uncertainty about the semivariogram, as well as due to uncertainty about the spatial distribution of NO$_3$-N within the agricultural field given the semivariogram and the available data from that field.

```{r, echo = FALSE}
load(file = "results/SimulatedSamplingVariances_Melle.RData")
```
...
As a model-based prediction of the sampling variance we can take the mean or the median of the sampling variances over all $100 \times 100$ simulated maps, which are equal to `r round(Exi_V_STSI,3)` (dag kg^-1^) and `r round(P50_V_STSI,3)` (dag kg^-1^), respectively.  If we want to be more safe, we can take a high quantile, e.g. the P90 of this distribution as the predicted sampling variance, which is equal to `r round(P90_V_STSI,3)` (dag kg^-1^).
...
```{r}
mysample <- read.csv("data/Sample_Xuancheng.csv")
...
```{r}
grdXuancheng <- readRDS(file = "data/grdXuancheng.rds")
res <- predict(lm_lnSOM, newdata = as(grdXuancheng, "data.frame"), se.fit = TRUE)
grdXuancheng <- within(as(grdXuancheng, "data.frame"), {
...
(ref:predictedlnSOMlabel) Predicted natural log of the SOM concentration (g kg^-1^) in the topsoil of Xuancheng and its standard error, obtained with a linear regression model with elevation as a predictor.
...
library(gstat)
mysample$s1 <- mysample$s1 / 1000
mysample$s2 <- mysample$s2 / 1000
...
dGeoR <- as.geodata(obj = mysample, header = TRUE,
   coords.col = c("s1", "s2"), data.col = "SOM_A_hori", covar.col = "dem"
 )
vgm_REML <- likfit(
...
Finally, as a preparatory step we must specify how precise the estimated mean should be. This precision must be specified in terms of the coefficient of variation (cv), i.e. the standard error of the estimated mean divided by the mean. I use a cv of 0.005. In case of multiple domains of interest and multiple study variables a cv must be specified per domain and per study variable. This precision requirement is used to compute the sample size for Neyman allocation (Equation \@ref(eq:Neymanallocation))^[For multivariate stratification, i.e. stratification with multiple study variables, Bethel allocation is used to compute the required sample size.]. The optimal stratification is independent of the precision requirement, although for a large cv the optimal number of strata can be smaller than the value passed to argument `nStrata` of function `optimStrata`, see hereafter.
...
The optimal spatial stratification can be computed with function `optimStrata`, with argument `method = "spatial"`. The $R^2$ value of the linear regression model, used in the minimisation criterion (Equation \@ref(eq:minicritospatsDeGruijter)), can be passed to argument `fitting`.
...
grdXuancheng <- as(grdXuancheng, "data.frame")
preds <- predict(lm_lnSOM, grdXuancheng)
