...
bounds <- apply(grdAmazonia[,covs], MARGIN=2, FUN=function(x) quantile(x, probs=probs, type=3))
ggplot(data=grdAmazonia) +
...
grdAmazonia$id <- 1:nrow(grdAmazonia)
...
#  time <- system.time(myClusters <- kmeanspp(scale(grdAmazonia[,covs]),k=n,iter.max=10000,nstart=40))
  time.CSC[i] <- time[1]
