...
In the next code chunk the minimized value of the criterion is computed "by hand".
...
In Figure \@ref(fig:RelationO1O3RMSE) the RMSE is plotted against the minimised O1 + O3 criterion for the cLH and the SI samples. For all three sample sizes there is a weak positive correlation of the minimisation criterion and the RMSE: for $n=25$, 50 and 100 this correlation is 0.363, 0.225, and 0.163, respectively. On average cLH performs slightly better than SI for $n=25$; for $n=50$ and 100, they perform about equal (Table \@ref(TableRMSE4cLHandSI)). Similar to covariate space coverage sampling (Section \@ref(PerformanceCSC)), especially for $n=25$ and 50 the distribution of RMSE with SI has a long right tail. For these small sample sizes the risk of selecting an SI sample leading to a poor map with large RMSE is much larger than with cLH sampling.


```{r RelationO1O3RMSE, echo=FALSE, fig.asp=0.7, fig.cap="Relation between minimisation criterion O1 + O3 and the root mean squared error (RMSE) of random forest predictions of aboveground biomass in Eastern Amazonia for conditioned Latin hypercube  sampling (clusters of points) and simple random sampling of three sample sizes. Solid lines are the fitted simple linear regression models for RMSE with O1 + O3 as a predictor."}
load("results/CSCversusCLH_Amazonia_n25.RData")
