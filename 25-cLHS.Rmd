...
```{r LatinSquare, echo=FALSE, out.width="50%",fig.cap="Latin hypercube sample for agricultural experiment with four application rates of N and P."}
...
Figure \@ref(fig:cLHS) shows the selected sample in a map of SWIR2. In Figure \@ref(fig:cLHSscat) the sample is plotted in a biplot of Prec_dm against SWIR2. Each black dot in the scatter plot represents one grid cell in the population. The vertical and horizontal lines in this scatter plot are at the bounds of the marginal strata of SWIR2 and Prec_dm, respectively. The number of grid cells (the black dots in the scatter plot) between two consecutive vertical lines is constant, as well as the number of grid cells between two consecutive horizontal lines, i.e. the marginal strata have equal sizes. The intervals are the narrowest where the density of grid cells in the plot is highest. Ideally, in each column and row there is exactly one sampling unit.
...
bounds <- apply(gridAmazonia[,covs], MARGIN=2, FUN=function(x) quantile(x, probs=probs, type=3))
ggplot(data=gridAmazonia) +
...
```{r}
probs <- seq(from=0, to=1, length.out=nrow(myCLHsample)+1)
bounds <- apply(gridAmazonia[,covs], MARGIN=2, 
...
bounds <- apply(subgrid[,covs], MARGIN=2, FUN=function(x) quantile(x, probs=probs, type=3))
