...
cLH samples can be selected with package **clhs** [@Roudier2011]. With this package the criterion is minimised by simulated annealing, see Section \@ref(SSA) for an explanation of this optimisation method. Arguments `iter`, `temp`, `tdecrease`, and ` length.cycle` are control parameters of the simulated annealing algorithm. I used default values. With argument `weights` the weights of the components of the minimisation criterion can be set. Default weights are 1.
...
set.seed(314)
res <- clhs(gridAmazonia[,covs], size=20, iter=1000, temp=1, tdecrease=0.95,
...
Figure \@ref(fig:cLHS) shows the selected sample in a map of SWIR2. Figure \@ref(fig:cLHSscat) shows the sample plotted in a scatter diagram of Precipitation Dryest Month against SWIR2. Each black dot represents one grid cell in the population. The horizontal and vertical lines in this scatter diagram are at the breaks of the marginal strata of SWIR2 and Precipitation Dryest Month, respectively. The number of black dots in between two consecutive vertical lines and in between two consecutive horizontal lines is constant (i.e. marginal strata have equal size). The intervals therefore are the narrowest where the density of black dots in the plot is highest. Ideally in each column and each row there is exactly one sampling unit.
...
```{r cLHSscat, echo=FALSE, fig.cap="Conditioned Latin hypercube sample plotted in a scatter diagram of SWIR2 against precipitation in dryest month. The horizontal and vertical lines are at the breaks of the marginal strata of the covariates SWIR2 and precipitation dryest month, respectively."}
...
```{r StratumSampleSizes, out.width='100%', fig.asp=0.4, fig.cap="Sample sizes of marginal strata for the conditioned Latin hypercube sample of size twenty from Eastern Amazonia."}
...
For all marginal strata\index{Marginal strata} with one sampling unit the contribution to component O1 of the criterion is zero. For marginal strata with zero or two sampling units, the contribution is 1, for marginal strata with three sampling units the contribution equals 2. In Figure \@ref(fig:StratumSampleSizes) there are nine marginal strata with zero units and nine marginal strata with two units, so O1 equals 18 in this case.
...
Package **clhs** can also be used for selecting a conditioned Latin hypercube sample in addition to existing sampling units, as in spatial infill sampling (Section \@ref(SpatialCoverage)). For this the argument `include` can be used. The argument size must then be set to the total sample size, i.e. the number of mandatory units plus the number of additional infill units.

To illustrate conditioned Latin hypercube *infill* sampling (cLHIS), I selected randomly ten units from Eastern Amazonia to serve as existing data (legacy sample). Twenty new units are selected by cLHIS. The ten mandatory units (i.e. already sampled and thus must be in the sample set computed by cLHIS) are at the end of the vector with the index of the selected raster cells.

```{r, eval=FALSE}
set.seed(314)
units <- sample.int(nrow(gridAmazonia), 10, replace=FALSE)
res <- clhs(gridAmazonia[,covs], size=30, include=units,
  tdecrease=0.95, iter=10000, progress=FALSE, simple=FALSE)
index <- res$index_samples
...
```{r cLHISscat, echo=FALSE, fig.asp=0.7, fig.cap="Conditioned Latin hypercube infill sample plotted in scatter diagram of SWIR2 against precipitation in dryest month. Legacy units have free-value 0, infill units have free-value 1."}
...
Figure \@ref(fig:boxplotsval) shows the results as boxplots, each based on 500 estimates. For $n=25$ and $100$ cLH sampling performs best in terms of RMSE and MEC, whereas for $n=50$ CSC sampling performs best. For $n=25$ and $50$ the boxplots of cLH and simple random sampling show quite a few outliers with large values of RMSE, resulting in small values of MEC. For CSC these map quality indices are more stable. Remarkably, for $n=100$ SI sampling performs about equal to CSC and cLH sampling.
...
In Figure \@ref(fig:RelationO1O3RMSE) the RMSE is plotted against the minimised O1 + O3 criterion for the cLH and the SI samples. For all three sample sizes there is a weak positive correlation of the minimisation criterion and the RMSE: for $n=25$, 50 and 100 this correlation is 0.363, 0.225, and 0.163, respectively. On average cLH performs slightly better than SI for $n=25$; for $n=50$ and 100, they perform about equal (Table \@ref(tab:TableRMSE4cLHandSI)). Similar to covariate space coverage sampling (Section \@ref(PerformanceCSC)), especially for $n=25$ and 50 the distribution of RMSE with SI has a long right tail. For these small sample sizes the risk of selecting an SI sample leading to a poor map with large RMSE is much larger than with cLH sampling.


```{r RelationO1O3RMSE, echo=FALSE, fig.asp=0.7, fig.cap="Relation between minimisation criterion O1 + O3 and the root mean squared error (RMSE) of random forest predictions of aboveground biomass in Eastern Amazonia for conditioned Latin hypercube (cLH)  sampling and simple random (SI) sampling of three sample sizes."}
load("results/CSCversusCLH_Amazonia_n25.RData")
...
rownames(df) <- NULL
knitr::kable(
  df, caption = 'Mean RMSE of random forest predictions of aboveground biomass in Eastern Amazonia of 500 conditioned Latin hypercube (cLH) samples and 500 simple random (SI) samples, for three sample sizes.',
  booktabs = TRUE,
