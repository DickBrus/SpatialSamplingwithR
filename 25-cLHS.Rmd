...
set.seed(314)
res <- clhs(gridAmazonia[,covs], size=20, tdecrease=0.95,
...
cLH samples can also be selected with function `optimCLHS` of package **spsann** [@Alessandro2016]. With this package the criterion is minimised by a spatial version of simulated annealing\index{Simulated annealing}. The function `scheduleSPANN` is used to define the annealing schedule\index{Annealing schedule}, see Chapter \@ref(MBSamplePattern) for details. My experience is that package **clhs** is much quicker than package **spsann**, and can handle larger data sets. The function `clhs` of this package has alos other interesting argument, such as the argument cost for cost-constrained cLH sampling [@Roudier2012], and `eta` to control the sampling intensities of the marginal strata [@Minasny2010]. The argument `eta` is of interest if we would like to oversample the marginal strata near the edge of the multivariate distribution.
