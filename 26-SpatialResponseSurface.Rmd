...
g_crf <- g_crf[v_crf]
g_crf <- st_intersection(v_crf, g_crf)
...
#plot(variogram1m, vgm1m)
#plot(variogram05m, vgm05m)

# subsample EM data, too many for prediction
# @DICK: subsampling is hier niet nodig, eventueel wel bij het berekenen van 
# de sample semivariogrammen (zie boven)
# @DICK: strippen van projectieattributen is ook niet nodig (wil je dat toch, dan 
# kun je onderstaande commentaar tekens weghalen)
#v_dat <- v_dat %>%
...
grdCRF <- EMpred %>%
    select(lnEM100cm = var1.pred) # %>%
#    as_tibble(EMpred) # het is de vraag of je dit nog wilt (een sf is ook een tibble)
