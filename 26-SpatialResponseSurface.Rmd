...
shpCRF <- readOGR(dsn = "data", layer = "CottonResearchFarm", verbose=FALSE)
...
EM_CRF <- as(EMpred,"data.frame")
EM_CRF <- EM_CRF[,-4]
names(EM_CRF)[3] <- "lnEM100cm"
...
EM_CRF$lnEM50cm<-df[,3]
EM_CRF$subarea <- mygrid$subarea

#save(EM_CRF, file="results/OKpred_EM_CRF.RData")
save(EM_CRF, file="data/CottonResearchFarm.RData")
```
...
for (i in 1:nrow(ccd_df)) {
    d2dpnt <- sqrt((EM_CRF$PC1-ccd_df$x1[i])^2+(EM_CRF$PC2-ccd_df$x2[i])^2)
    EM_CRF <- EM_CRF[order(d2dpnt),]
    candi <- EM_CRF[c(1:8), c("unit","x1","x2","PC1","PC2")]
    candi$dpnt <- i
...
for (i in 1:nrow(ccd_df)) {
    d2dpnt <- sqrt((EM_CRF$PC1-ccd_df$x1[i])^2+(EM_CRF$PC2-ccd_df$x2[i])^2)
    EM_CRF <- EM_CRF[order(d2dpnt),]
    candi <- EM_CRF[c(1:16), c("unit","x1","x2","PC1","PC2")]
    candi$dpnt <- i
    candi_all <- rbind(candi_all,candi)
}
```
