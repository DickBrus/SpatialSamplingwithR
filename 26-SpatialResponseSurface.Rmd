...
Figure \@ref(fig:candidatelocations) shows a scatter plot of the two standardised principal component scores of all 1,552 grid cells.
...
The candidate sampling units are selected in a double for-loop. The outer loop is over the strata, the inner loop over the design points. Note that variable `dpnt` continues to increase by 1 after the inner-loop over the nine design points in subarea 1 is completed, so that variable `dpnt` (used as a stratum in subsampling the sample of candidate sampling points) now has values $1,2, \cdots , 18$.  Stratum 2 is somewhat larger than stratum 1. To select an equal number of candidate sampling points per design point in both strata (eight points), different values for argument `p` of function `quantile' are used for the two strata. 
...
for (h in c(1,2)) {
  ...
  for (i in 1:nrow(ccd_df)) {
      ...
      units_close <- which(d2dpnt < q)
      candi <- data_stratum[units_close, c("unit","x1","x2","PC1","PC2")]
...
```{r, echo=FALSE, fig.cap="Candidate sampling points for stratified CCRSD sampling, plotted on a map of the first principal component (PC1)."}
