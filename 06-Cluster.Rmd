...
As our population actually is infinite, the centres of the selected grid cells are jittered to a random point within the selected grid cells. Note that the same noise is added to all points of a given cluster.
...
Figure \@ref(fig:SamplingDistributionCl) shows the approximated sampling distribution of the pwr estimator of the mean with cluster random sampling, and of the $\pi$ estimator with simple random sampling, approximated by repeating the random sampling with each design and estimation 10,000 times. The size of the simple random samples is equal to the expected sample size of the cluster random sampling design (rounded to nearest integer).
...
```{r histsamplesizeCl, echo=FALSE, fig.cap="Approximated sampling distribution of the sample size (number of points) with cluster random sampling from Voorst, for six clusters selected by ppswr."}
