...
Define residual variogram (see Table 23.1).
...
```{r}
#read discretisation grid with interpolated EM values
load(file="data/CottonResearchFarm.RData")
set.seed(314)
units <- sample.int(nrow(EM_CRF), size=1000, replace=TRUE)
mysample <- EM_CRF[units,]

#Shift the randomly selected grid points to random points within the cells (the cellsize is 25 m x 25 m)
...
Choose grid spacings to be evaluated. Take 100 m as the largest spacing. The farm is about 100 ha, so with this spacing the number of grid points is on average about 100. Take 70 m as the smallest spacing. Select a square grid with the first spacing, add a dummy variable and predict the dummy variable at the prediction points by kriging with an external drift. Compute the average of the kriging variances at the 1,000 prediction points, and save it in a matrix. Repeat this several times (inner for-loop), and repeat this for the other spacings (outer for-loop).
...
```
Compute tolerable grid spacing for a MMKV of 0.170.
