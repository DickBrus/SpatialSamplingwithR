...
Select a simple random sample of size 1,000 for evaluating the square grids. Add a small number to the x-coordinates and y-coordinates by drawing from a uniform distribution with lower and upper limit equal to -cellsize/2 and +cellsize/2, respectively. This can be done with function jitter.
...
Specify the grid spacings to be evaluated. Specify the cumulative probabilities from which the corresponding quantiles of the ordinary kriging variance at the 1,000 evaluation points are computed. Save these quantiles in a numeric. Select a square grid with the first spacing, add a dummy variable and predict the dummy variable at the evaluation points by ordinary kriging.  Repeat this for the other spacings (for-loop). Finally plot the quantiles of the kriging variance against the grid spacing.
...
Change the nugget parameter to 0.6, and change the partial sill parameter so that the sill (nugget + partial sill) is unchanged. Determine the tolerable grid spacing. Explain the difference in determined grid spacing.
...
Do the same for a smaller range. Change the range to 10 km. Explain the difference in grid spacing.
...
The relation between the mean kriging variance and the grid spacing obtained with the three variograms is used to compute the tolerable grid spacing for a MKV value of 0.85.

```{r}
res1 <- lapply(1:3, function (i) approx(x=df[,i+1], y=df$spacing, xout=0.85)$y) %>% as.numeric(.)
