...
load(file = "../results/SimulatedSquare.rda")
...
set.seed(314)
res <- clhs(grdSquare[, c(1, 2, 3)], size = 16, tdecrease = 0.95, iter = 10000, progress = FALSE, simple = FALSE)
index <- res$index_samples
myclhsample <- grdSquare[index, ]
...
breaks <- apply(grdSquare[, c(1, 2, 3)], MARGIN = 2, FUN = function(x) quantile(x, probs = probs))
...
ggplot(tracedf) +
  geom_line(mapping = aes(x = seq_len(nrow(tracedf)), y = trace), colour = "black", size = 0.8) +
