...
for (i in 1:10000) {
  ...
  mz_reg2ph[i] <- mean(mysubsample$AGB)+coef(lm_subsam)[2]*(mean(mysample$lnSWIR2)-mean(mysubsample$lnSWIR2))
  
  #approximate the sampling variance of the regression estimator
  av_mz_reg2ph[i] <- (1-n1/N)*var(mysubsample$AGB)/n1 + (1-n2/n1)* (sum(lm_subsam$residuals^2)/(n2-2))/n1
}
