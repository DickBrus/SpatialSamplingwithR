...
ggplot(data = grdKandahar) +
  geom_raster(mapping = aes(x = s1, y = s2, fill = agri)) +
...
1. Write an **R** script to select a pps with replacement sample from Eastern Amazonia to estimate the population mean of aboveground biomass (AGB), using log-transformed short-wave infrared (SWIR2) as a size variable. Use the 5 km $\times$ 5 km subgrid to reduce computing time (data are in `results`/grdAmazonia_5km.rds`).      
