...
1. Use function `strata` of package **sampling** to select a stratified simple random sample with replacement of size 40 from Voorst, using proportional allocation. Check that the sum of stratum sample sizes is 40.  
...
5. Write an **R** script to compute five *cum-root-f* strata for Eastern Amazonia (`data.frame grdAmazonia` in package **sswr**) to estimate the population mean of aboveground biomass (AGB), using log-transformed short-wave infrared (SWIR2)  as stratification variable.   
...
```{r, eval = FALSE, echo = FALSE}
saveRDS(mygeostrata, file = "results/geostrata_Xuancheng.rds")
```

```{r, echo = FALSE}
subgrd <- as(subgrd, "data.frame")
mygeostrata <- readRDS(file = "results/geostrata_Xuancheng.rds")
```
...
Operator `%>%` of package **magrittr** [@magrittr] can be used to merge the two lines in the code chunk above. In this way we can save memory, as we do not need an object of class `SamplingPatternRandomSamplingUnits` obtained with function `spsample`.
...
9. Write an **R** script to construct 20 compact geographical strata of equal size for agricultural field Leest the shape file of this field can be read with ...... Remove the projection attributes with `proj4string(shpLeest) <- NA_character_`. Select two points per geostratum, using function `spsample` of package **spcosa**. Repeat this with 40 strata of equal size, and randomly select one point per stratum.  
...
```{r OptimisedstrataMeuse, echo = FALSE, fig.width = 5,  fig.cap = "Map of optimised bivariate strata of study area Meuse."}
