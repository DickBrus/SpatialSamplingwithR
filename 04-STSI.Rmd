...
The stratum sample sizes must be in the order in which the strata are encountered in `data.frame` `grdVoorst`, which is determined first with function `unique`.
```
...
```{block2, type = 'rmdnote'}
The name of the package is added to function (`sampling::strata`), as `strata` is also a function in another package. Not adding the name of the package may result in an error message.
```
...
df <- data.frame(nms, Nh = N_h, nh = n_h_copy, Mean = round(mz_h, 1), Var = round(S2z_h, 1), se = round(sqrt(v_mz_h), 1))
...
Table \@ref(tab:stratummeans) shows per stratum the estimated mean, the estimated variance, and the estimated sampling variance of the estimated mean of the soil organic matter (SOM) concentration. We can see large differences in the within-stratum variances\index{Within-stratum variance}. For the stratified sample of Figure \@ref(fig:SampleSTSI) the estimated population mean equals `r round(mz,1)` g kg^-1^ and the estimated standard error of this estimator equals `r round(se_mz,1)` g kg^-1^.

The population mean can also be estimated directly using the basic $\pi$ estimator (Equation \@ref(eq:HTMean)). The inclusion probabilities are included in `data.frame` \texttt{mysample}, obtained with function `getdata` (see code chunk above), in column `Prob`.
...
```{r EstimatedCDFVoorstSTSI, echo = FALSE, fig.asp = 0.7, fig.cap = "(ref:EstimatedCDFVoorstSTSIlabel)"}
dx <- mean(diff(df$x))
dat <- data.frame(
    x = c(df$x[1] - dx, df$x),
...
1. we are interested in the mean (total) per stratum; or
...
```{r SamplingDistributionSTSI, echo = FALSE, fig.asp = .8, fig.cap = "(ref:SamplingDistributionSTSIlabel)"}
...
In other words, the smaller the differences in the stratum means and the larger the variances within the strata, the smaller the stratification effect will be. Figure \@ref(fig:boxplotsSOMstrata) shows a boxplot of the SOM concentration per stratum (soil-land use combination). The stratum means are equal to `r round(mz_h_pop,1)` g kg^-1^. The stratum variances are `r round(S2z_h_pop,1)` (g kg^-1^)^2^. The large stratum variances explain the modest gain in precision realised by stratified simple random sampling compared to simple random sampling in this case.
...
taballoc <- data.frame(Stratum = strata, Nh = as.numeric(N_h), Sh = as.numeric(round(sqrt(S2z_h), 1)), nhprop = as.numeric(n_h), nhNeyman = as.numeric(n_h_Neyman))
...
1. Load `data.frame` `grdVoorst`, and use function `strata` of package **sampling** to select a stratified simple random sample with replacement of size 40, using proportional allocation. Check that the sum of stratum sample sizes is 40.  
...
```{r DEMXuancheng, echo = FALSE, fig.cap = "(ref:DEMXuanchenglabel)"}
...
The strata can be constructed with the package **stratification** [@Baillargeon2011]. Care should be taken that the data are sorted in ascending order by the columns used for stratification, see help of function `strata.cumrootf`. Argument `n` of this function is the total sample size, but this value has no effect on the stratification. Argument `Ls` is the number of strata. I arbitrarily chose to construct five strata.  Argument `nclass` is the number of bins of the histogram. The output object of function `strata.cumrootf` is a list containing amongst others a numeric vector with the stratum bounds (`bh`) and a factor with the stratum levels of the grid cells (`stratumID`). Finally, note that the values of the stratification variable must be positive. The minimum elevation is -5 m, so we added the absolute value of this minimum to elevation.
...
Stratum bounds are threshold values of the stratification variable elevation; these stratum bounds\index{Stratum bound} are equal to `r round(bh,1)`. Note that the number of stratum bounds is one less than the number of strata. The resulting stratification is shown in Figure \@ref(fig:optstrata). Note that most strata are not single polygons, but are made up of many smaller polygons. This may be even more so if the stratification variable shows a noisy spatial pattern. This is not a problem at all, a stratum is just a collection of population units (raster cells) and need not be spatially contiguous.

(ref:optstratalabel) Stratification of Xuancheng obtained with *cum-root-f* method, using elevation as a stratification variable.

```{r optstrata, echo = FALSE, fig.cap = "(ref:optstratalabel)"}
...
subgrd <- spsample(grdXuancheng, type = "regular", cellsize = 400, offset = c(0.5, 0.5))
...
(ref:kmeansstrataXuanchenglabel) Five clusters obtained  by k-means clustering of the raster cells of Xuancheng, using five scaled covariates.

```{r kmeansstrataXuancheng, echo = FALSE, fig.cap = "(ref:kmeansstrataXuanchenglabel)"}
ggplot(subgrd) +
...
Operator `%>%` of package **magrittr** [@magrittr] can be used to merge the two lines in the code chunk above. In this way we can save memory, as we do not need an object of the class `SamplingPatternRandomSamplingUnits` obtained with function `spsample`.
...
Figure \@ref(fig:GeoStrata) shows fifty compact geostrata of equal size for Xuancheng with the selected sampling points.  Note that the sampling points are reasonably well spread throughout the study area.

```{r GeoStrata, echo = FALSE, fig.cap = "Compact geostrata of equal size for Xuancheng and stratified simple random sample of two points per stratum."}
ggplot(mygeostrata) +
...
The estimated population mean equals `r round(mean,1)` with an estimated standard error of `r round(se,1)`.
...
9. Write an **R** script to construct 20 compact geographical strata of equal size for agricultural field Leest. Read the shapefile `Leest` using function `readOGR` of the package **rgdal**. Remove the projection attributes with `proj4string(shpField) <- NA_character_`. Select two points per geostratum, using function `spsample` of package **spcosa**. Repeat this with 40 strata of equal size, and randomly select one point per stratum.  
...
```{r PredictedCd, echo = FALSE, out.width = '100%', fig.cap = "Kriging predictions of natural logs of Cd and Zn concentrations in study area Meuse, used as stratification variables in bivariate stratification."}
