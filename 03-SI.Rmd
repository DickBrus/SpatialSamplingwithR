...
In **R** a simple random sample can be selected with or without replacement by the function `sample` from the **base** package. For instance, a simple random sample without replacement of 10 units from a population of 100 units labelled as 1,2, ... ,100, can be selected by
...
The result of the function `sample` is a vector with the selected nodes of the discretisation grid. The order of the elements of the vector is the order in which these are selected. Restricting the sampling points to the nodes of a discretisation grid can be avoided as follows. The columns `s1` and `s2` in the `data.frame grdVoorst` contain the spatial coordinates of the centres of grid cells of 25 m $\times$ 25 m. Now a simple random sample is selected in two stages. First, *n* times a grid node is selected by simple random sampling *with replacement*. Second, every time a grid node is selected, one point is selected fully randomly from the grid cell with the node in its centre. This selection procedure accounts for the infinite number of points in the population. In the code chunk below the second step of this selection procedure is implemented with function `jitter`. It adds random noise to the spatial coordinates of the centres of the selected grid cells, by drawing from a continuous uniform distribution $\text{unif}(-c,c)$, with $c$ half the side length of the square grid cells. With this selection procedure we respect that the population actually is infinite.  
...
The estimated total is  17,903 ton.
...
```{r SamplingDistributionSI, echo=FALSE, fig.asp=0.8, fig.cap="Sampling distribution of the estimator of the mean of SOM (g kg$\\textsuperscript{-1}$) in Voorst for simple random sampling of size 40."}
n <- 40
...
```{r CDFSIVoorst, fig.asp=0.7, fig.cap="Estimated cumulative distribution function of SOM (g kg$\\textsuperscript{-1}$), estimated from the simple random sample of 40 units from Voorst."}
...
```{r coverageconfinterval, echo=FALSE, fig.cap="Estimated confidence intervals of the mean of SOM (g kg$\\textsuperscript{-1}$) in Voorst, estimated from 100 simple random samples of size 40. The vertical red line is at the true population mean."}
...
8. Write an **R** script to select a simple random sample of size 40 from Voorst (data are in `data/Voorst.RData`).
    ...
    + Use the selected sample to estimate the total mass of SOM in tons in the topsoil ($0-30$ cm) of Voorst. Use as a bulk density 1,500 kg m$^{-3}$. The size of the grid cells is 25 m $\times$ 25 m.
