...
where $n$ is the number of draws (samle size), and $p_{k}$ is the draw-by-draw selection probability of unit $k$. With simple random sampling $p_{k}=1/N, k=1, \dots , N$. Inserting this in the pwr estimator yields
...
8. Write an **R** script to select a simple random sample of size 40 from Voorst (data are in `data/Voorst.RData`).  
    ...
    + Use the selected sample to estimate the total mass of soil organic matter in megatons in the topsoil (0 - 30 cm) of Voorst. Use as a bulk density 1500 kg m$^{-3}$. The size of the pixels is 25 m by 25 m.  
