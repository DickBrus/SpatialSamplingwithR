...
\begin{eqnarray}
Z_{dk} &=& \mathbf{x}_{dk}^{\text{T}} \symbfit{\beta} + v_d + \epsilon_{dk} \notag \\
...
with $\hat{\symbfit{\beta}}$ the best linear unbiased estimates (BLUE) of the regression coefficients, and $\hat{v}_d$ the best linear unbiased prediction (BLUP) of the intercept for domain $d$, $v_d$. The model-based predictor can also be written as
