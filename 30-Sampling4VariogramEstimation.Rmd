...
The data frame file `allpairs` has four columns: the spatial coordinates of the first and of the second point of a pair. An overlay is made of the selected points with the `SpatialPixelsDataFrame` and the cti (compound topographic index, which is the same as topographic wetness index) values are extracted.
...
## Model-based optimisation of pattern of a single sample for both semivariogram estimation and prediction {#SamplingEstimationandPrediction}
