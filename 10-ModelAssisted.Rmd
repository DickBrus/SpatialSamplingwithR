...
1. The data for this exercise are in `data/Amazonia_1km.RData`. Write an **R** script to
    + Select a simple random sample without replacement of size 50. Compute the regression estimator of the population mean of AGB and its standard error "by hand", using lnSWIR2 as a covariate (so do not not use package **survey**). Use Equation \@ref(eq:VarianceRegressionEstimatorSI) to estimate the standard error.   
...
The working model\index{Working model} of the ratio estimator\index{Ratio estimator} is a heteroscedastic model, i.e. a model with non-constant variance (see Exercise 3, hereafter):
