...
rmap <- rast(x="data/Geo_Xuancheng.tif")
...
These four new variables now are our study variables of which we would like to estimate the population means. The population means can be estimated as explained in Chapter \@ref(STSI). First, the stratum weights are computed, that is the relative number of raster cells.



```{r}
strataXuancheng <- strataXuancheng %>%
...
me <- sample_test %>%
    ...
    summarise(
      meKED_h = mean(eKED),
      meRF_h = mean(eRF)) %>%
    left_join(strataXuancheng, by = "stratum") %>%
    summarise(
...
```{r}
strataXuancheng_clp <- strataXuancheng %>%
...
se <- sample_test %>%
    ...
    summarise(
      ...
      s2e_RF_hc = var(eRF),
      s2e2_RF_hc = var(e2RF)) %>%
    left_join(strataXuancheng_clp, by = "stratum_clp") %>%
    summarise(
...
m_de2 <- sum(strataXuancheng$w_h * m_de2_h)

s2_de2_hc <- tapply(sample_test$de2, INDEX = sample_test$stratum_clp, FUN = var)
se_m_de2 <- sqrt(sum(strataXuancheng_clp$w_hc^2 * s2_de2_hc / strataXuancheng$clp$n_hc))
t <- m_de2 / se_m_de2
