...
To estimate MEC, we must first estimate the population variance of the study variable from the stratified simple random sample (the denominator in Equation \@ref(eq:HTMEC)). This population variance is estimated with function `s2` of package **surveyplanning** (Subsection \@ref(WhyStratify)).
...
The estimated MEC for KED equals `r round(mec_KED,3)` and for RF `r round(mec_RF,3)`, showing that the two models used in mapping are no better than the estimated mean SOM concentration used as a predictor. This is quite a disappointing result.
...
Now we test whether the two population MSEs  differ significantly. This can be done by a paired *t*-test\index{\emph{t}-test!paired \emph{t}-test}. The first step in a paired *t*-test is to compute pairwise differences of squared predictions errors, and then we can proceed as in a one-sample *t*-test. 
