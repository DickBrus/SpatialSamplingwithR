...
Estimation of the class representations is slightly more difficult because the sizes of the classes (number of raster cells or area where in reality class $u$ is present) are unknown, and therefore these sizes must also be estimated from the sample. This leads to the ratio estimator:
...
```{r validationsample, echo = FALSE, fig.cap = "Stratified simple random sample for validation of the two maps of the SOM concentration in Xuancheng."}
library(terra)
...
The collapsed strata\index{Collapsed strata} can be used to estimate the standard errors of the estimated population MSEs. As a first step the weights and the sample sizes of the collapsed strata are computed.
