...
with $c_k$ and $\hat{c}_k$ the true and predicted class of unit $k$, respectively. For infinite populations the purity is the fraction of the area that is correctly classified (mapped).
...
Estimation of the class representations is slightly more difficult because the sizes of the classes (number of pixels or area where in reality class $u$ is present) are unknown, and therefore must also be estimated from the sample. This leads to the ratio estimator:
...
This estimator is also recommended for estimating other map quality indices from a sample with a sample szie that is not fixed but varies among  samples selected with the sampling design. This is the case, for instance, when estimating the mean (absolute or squared) error or purity of a given map unit from a simple random sample. The number of selected sampling units within the map unit is uncontrolled and varies among the simple random samples. In this case we can either estimate the mean error or purity of a map unit $u$ by dividing the estimated population total by the *known* size (area, number of pixels) of map unit $u$, or by the *estimated* size. Interestingly, using the estimated size in the denominator instead of the known size, yields in general a more precise estimator [@sar92]. See also Section \@ref(LargeDomainsDirectEstimator). 
...
sample_all <- read.csv("data/Sample_Xuancheng.csv")
sample_train <- sample_all[sample_all$sample %in% c("grid","iPSM"),]
...
sample_test <- sample_all[sample_all$sample == "STSI", ]
...
sample_trn <- as(sample_trn,"data.frame")
dGeoR <- as.geodata(
  obj=sample_trn, 
  header=TRUE, 
  coords.col=c("s1", "s2"), 
...
saveRDS(grdXuancheng,file = "data/grdXuancheng.rds")
```
...
```{r validationsample, echo=FALSE, fig.cap="Stratified simple random sample for validation of the two maps of the SOM concentration in Xuancheng, China."}
library(terra)

rmap <- rast("data/Geo_Xuancheng.tif")
grd <- as.data.frame(rmap, xy=TRUE, na.rm=TRUE)
units <- which(grd$Geo_Xuancheng == 99)
grd <- grd[-units,]
...
ggplot(data=grd) +
  ...
  scale_x_continuous(name = "Easting (km)") +
  scale_y_continuous(name = "Northing (km)") +
  geom_point(data=sample_test,mapping = aes(x=s1/1000,y=s2/1000),size=2, colour="red") +
coord_fixed()
```
...
```{r}
mystrata <- read.csv(file = "data/Strata_Xuancheng.csv") %>%
...
The estimated MSE of the KED map equals `r as.character(round(mse$mseKED,1))` (g kg^-1^)^2^, that of the RF map `r as.character(round(mse$mseRF,1))` (g kg^-1^)^2^.
...
We are uncertain about both population MSEs, as we measured the squared errors at `r nrow(sample_test)` sampling points only. So, we would like to know how uncertain we are. This uncertainty is quantified by the standard error of the estimated population MSE. A problem is that in the second stratum we have only one sampling point. So, for this stratum we cannot compute the variance of the squared errors. To compute the variance we need at least two sampling points. 
...
A solution is to merge stratum 2 with stratum 1, which is a similar geological map unit (we know this from the domain expert). This is referred to as collapsing the strata. An identifier for the collapsed strata is added to the look-up table `lut`. This table is subsequently used  to add the collapsed strata identifiers to the `data.frame`s `sample_test` and `mystrata`.
...
The collapsed strata\index{Collapsed strata} can be used to estimate the standard errors of the estimated population MSEs. As a first step the stratum weights and the sample sizes of the collapsed strata are computed.

```{r}
mystrata_clp <- mystrata %>%
...
The sampling variance of the estimator of the mean of the (squared) prediction error can be estimated by Equation \@ref(eq:EstVarMeanSTSI). The estimated MSE and MSE and their estimated standard errors are shown in Table \@ref(tab:validationresults).
...
se <- sample_test %>%
    ...
    summarise(
      ...
      s2e_RF_hc=var(eRF),
      s2e2_RF_hc=var(e2RF)) %>%
    left_join(mystrata_clp, by="stratum_clp") %>%
    summarise(
...
The estimated MEC for KED equals `r round(mec[1],3)` and for RF `r round(mec[2],3)`, showing that the two models used in mapping are no better than the estimated mean SOM concentration used as a predictor. This is quite a disappointing result.
...
m_de2 <- sum(mystrata$w_h*m_de2_h)

s2_de2_hc <- tapply(sample_test$de2, INDEX=sample_test$stratum_clp, FUN=var)
se_m_de2 <- sqrt(sum(mystrata_clp$w_hc^2*s2_de2_hc/mystrata$clp$n_hc))
t <- m_de2/se_m_de2
...
The outcome of the test statistic is `r as.character(round(t, 3))`, with a *p*-value\index{\emph{p}-value of a test!two-sided \emph{p}-value} of `r as.character(round(p,3))`, so we clearly do not have enough evidence that the population MSEs obtained with the two mapping methods are different.
