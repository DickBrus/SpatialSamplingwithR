...
All probability sampling designs described in Part I are in principle appropriate for validation. @steh99 evaluated five basic probability sampling designs and concluded that in general stratified random sampling is a good choice. For validation of categorical maps natural strata are the map units, i.e. the groups of polygons or grid cells assigned to each class. Systematic random sampling is less suitable as no unbiased estimator of the sampling variance of the estimator of a population mean exists for this design (see Chapter \@ref(SY)). For validation of maps of extensive areas, think of whole continents, travel time between sampling locations can become substantial. In this case sampling designs that lead to spatial clustering of validation locations can become efficient, for instance two-stage cluster random sampling (Chapter \@ref(Twostage)) or cluster random sampling (Chapter \@ref(Cl)).

## Real-world case study

As an illustration two soil maps of the three northern counties of Xuancheng City (Anhui province, China), both depicting soil organic matter (SOM) concentration (g kg^-1^) in the topsoil, are evaluated. In Section \@ref(Ospats) the data of three samples, including the stratified random sample, were merged to estimate the parameters of a spatial model for the natural log of the SOM concentration. Here only the data of the two non-random samples are used to map the SOM concentration. The stratified simple random sample is used for validation.
...
mysample <- mysample %>%
    mutate(
      eKED=SOM_A_hori-KED,
      eRF=SOM_A_hori-RF,
      e2KED=(SOM_A_hori-KED)^2,
      e2RF=(SOM_A_hori-RF)^2)
```
...
```{r}
mystrata <- read.csv(file="data/Xuancheng_StrataSize.csv") %>%
...
1. The estimated MSE of the KED map equals `r as.character(round(mse$mseKED,1))` (g kg^-1^)^2^, that of the RF map `r as.character(round(mse$mseRF,1))` (g kg^-1^)^2^. Are you certain that the population MSE of the KED map is smaller than the population MSE of the RF map? 
...
The sampling variance of the estimator of the mean of the (squared) prediction error can be estimated by Equation \@ref(eq:EstVarMeanSTSI).
...
df <- nrow(mysample)-length(unique(mysample$stratum_clp))
...
```{r, echo=FALSE, eval=FALSE}
mysample$de2 <- mysample$e2KED-mysample$e2RF #mutate
m_de2_h <- tapply(mysample$de2, INDEX=mysample$stratum, FUN=mean) #group_by
m_de2 <- sum(mystrata$w_h*m_de2_h)

s2_de2_hc <- tapply(mysample$de2, INDEX=mysample$stratum_clp, FUN=var)
