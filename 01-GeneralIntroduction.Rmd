...
with $f(x)$ the proportion of population units whose value for the study variable $z$ equals $x$.
...
The study area of Voorst is located in the eastern part of the Netherlands. The size of the study area is 6 km $\times$ 1 km. At 132 points samples of the topsoil were collected by graduate students of Wageningen University, which were then analysed for soil organic matter (SOM) concentrations (in g/kg) in a laboratory. The map is created by conditional geostatistical simulation of natural logarithms of SOM on a 25 m $\times$ 25 m grid, followed by backtransformation, using a linear mixed model with spatially correlated residuals and combinations of soil type and land use as a qualitative predictor (factor). Figure \@ref(fig:mapVoorst) shows the simulated map of SOM. 

```{r mapVoorst, echo=FALSE,  out.width='100%', fig.cap="Simulated soil organic matter concentration (g/kg) in Voorst (Netherlands)."}
...
The histogram of the simulated values at all 7,528 grid cells shows that SOM is skewed to the right (Figure \@ref(fig:histogramVoorst)).
...
Cultivation of poppy for opium production is a serious problem in Afghanistan. The United Nations Organization on Drugs and Crime (UNODC) monitors the area cultivated with poppy through detailed analysis of aerial photographs and satellite images. This is laborious, and therefore the analysis is restricted to a probability sample of 5 km $\times$ 5 km squares. These sample data are then used to estimate the total poppy area [@UNODC2014].

In 2014 the poppy area within 83 randomly selected squares in the province of Kandahar (Afghanistan) was determined, as well as the agricultural area within all 965 squares in this province. These data were used to simulate a map of poppy area per 5 km $\times$ 5 km square. The map is simulated with an ordinary kriging model for the logit transform of the proportion of the agricultural area cultivated with poppy within a 5 km $\times$ 5 km square. For privacy reasons the field was simulated *unconditionally* on these sample data. Figure \@ref(fig:mapsKandahar) shows the map with the agricultural area in hectares per 5 km $\times$ 5 km square and the map with the simulated poppy area in hectares per square. The histogram of the simulated poppy area per square shows very strong positive skew (Figure \@ref(fig:histogramPoppyarea)). For 375 squares the simulated poppy area was smaller than 1 ha. 

```{r mapsKandahar, echo=FALSE, fig.cap="Agricultural area and simulated area cultivated with opium poppy area, in ha per 5 km $\\times$  5 km square, in Kandahar (Afghanistan)."}
...
```{r histogramPoppyarea, echo=FALSE, fig.asp=0.7, fig.cap="Histogram of simulated poppy area in ha  per 5 km $\\times$ 5 km square in Kandahar."}
ggplot(grdKandahar) +
...
### Aboveground biomass in Eastern Amazonia (Brazil)

This data set consists of data on the aboveground live woody biomass (AGB) in megatons per ha [@Baccini2012]. A rectangular area of 1,642 km $\times$ 928 km in Eastern Amazonia (Brazil) was selected from this data set, and aggregated to a map with a resolution of 1 km $\times$ 1 km. Besides, a stack of five ecologically relevant covariates of the same spatial extent was prepared, being MODIS long term mean of MODIS short-wave infrared radiation (SWIR2), Primary Production in kg C per m$^2$ (Terra_PP), average precipitation in driest month in mm (Prec_dm), Elevation in m and Clay content in g/kg soil.  All covariates were either resampled by bilinear interpolation or aggregated to conform with the grid of the above-ground biomass map. Figure \@ref(fig:mapsAmazonia) shows a map of AGB and SWIR2.

```{r mapsAmazonia, echo=FALSE, out.width='100%', fig.cap="Aboveground biomass in megatons per ha (AGB) and short-wave infrared radiation (SWIR2) of Eastern Amazonia (Brazil)."}
...
```{r matrixscatter, echo=FALSE, fig.asp= .66, out.width='100%', fig.cap="Matrix of two-dimensional density plots of aboveground biomass (AGB) and five covariates of Eastern Amazonia."}
library(GGally)
...
The space-time designs of Chapter \@ref(RepeatedSurveys) are illustrated with the annual mean air temperature at two metres above the earth surface in degrees Celsius, in Spain and Portugal (islands excluded) for the years 2004, 2009, 2014 and 2019. These data are part of the data set [CHELSA](https://chelsa-climate.org/wp-admin/download-page/CHELSA_tech_specification_V2.pdf) [@Karger2017]. The raster files are latitude-longitude grids with a resolution of 30 arc sec. These latitude-longitude grids are not suitable for sampling, as the area of the grid cells is not constant. The next code chunk shows how the raster file for 2004 can be projected using the Lambert azimuthal equal area (laea) projection, and how the class of the resulting raster file can be changed into a data frame with **R** package **terra** [@terra]. The resolution of the resulting laea raster file is about 780 m $\times$ 780 m.

```{r, eval=FALSE}
library(terra)
TAS2004 <- rast("data/TAS2004.tif")
TAS2004proj <- project(TAS2004, "epsg:3035", method="bilinear",
  filename="TAS2004proj.tif")
df_2004 <- as.data.frame(TAS2004proj, xy=TRUE, na.rm=TRUE)
```

```{r, echo=FALSE, eval=FALSE}
TAS2009 <- rast("data/TAS2009.tif")
TAS2014 <- rast("data/TAS2014.tif")
TAS2019 <- rast("data/TAS2019.tif")

TAS2009proj <- project(TAS2009, "epsg:3035", method="bilinear", filename="TAS2009proj.tif", overwrite=TRUE)
TAS2014proj <- project(TAS2014, "epsg:3035", method="bilinear", filename="TAS2014proj.tif", overwrite=TRUE)
TAS2019proj <- project(TAS2019, "epsg:3035", method="bilinear", filename="TAS2019proj.tif", overwrite=TRUE)

df_2009 <- as.data.frame(TAS2009proj, na.rm=TRUE)
df_2014 <- as.data.frame(TAS2014proj, na.rm=TRUE)
df_2019 <- as.data.frame(TAS2019proj, na.rm=TRUE)
df <- data.frame(x=df_2004$x, y=df_2004$y, TAS2004=df_2004$TAS2004, TAS2009=df_2009$TAS2009, TAS2014=df_2014$TAS2014, TAS2019=df_2019$TAS2019)
save(df, file="data/SpainPortugal_TAS.RData")
```

```{r TASofSpainPortugal, echo=FALSE, out.width='100%', fig.cap="Annual mean air temperature in Spain and Portugal for the years 2004, 2009, 2014 and 2019."}
