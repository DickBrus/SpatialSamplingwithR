...
1. Write an **R** script to select a square grid of size 100 from West-Amhara in Ethiopia. Use `data.frame` `grdAmhara`, stored in `data/grdAmhara.rds`, as a sampling frame. Use a fixed starting point of the grid, i.e. do not select the grid randomly.   
...
#### Exercises {-}
6. Write an **R** script to select a spatial infill sample of size 100 from study area Xuancheng in China. Use the iPSM sample in `data.frame` `data/Sample_Xuancheng.csv` as a legacy sample. To map the SOM concentration we want to measure the SOM concentration at 100 more sampling points. Use the raster file `data/Elevation_Xuancheng.tif` as a discretisation of the study area. This file can be read with function `rast` of package **terra**. 
