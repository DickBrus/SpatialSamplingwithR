...
1. Write an **R** script to select a square grid of size 100 from West-Amhara in Ethiopia. Use `data.frame` `grdEthiopia`, stored in `data/CovariatesAmharaEthiopia.RData`, as a sampling frame. Use a fixed starting point of the grid, i.e. do not select the grid randomly.   
...
ggplot(mystrata) +
    ...
    geom_point(data=mysample, mapping=aes(x=s1/1000, y=s2/1000), size=1) +
    coord_fixed() +
    scale_x_continuous(name="Easting (km)")+
    scale_y_continuous(name="Northing (km)")+
    theme(legend.position="none")
...
#### Exercises {-}
6. Write an **R** script to select a spatial infill sample of size 100 from study area Xuancheng in China. Existing data on SOM in the topsoil at 60 sampling points are in `data.frame` `data/Xuancheng_iPSMsample.csv`. To map the SOM concentration we want to measure the SOM concentration at 100 more sampling points. Use the raster file `data/Xuancheng_elevation.tif` as a discretisation of the study area. This file can be read with function `rast` of package **terra**. 
