...
The estimated population mean equals `r round(mz,2)`. The sampling variance over repeated sampling with both designs can be approximated^[In the approximation it is assumed that $N$ is much larger than $n_1$, and $(n_{1h}-1)/(n_1-1)$ is replaced by $n_{1h}/n_1$.] by (@sar92, Equation at bottom of p. 353)
...
The ratio's $(1-n_1/N)$ and $(1-n_2/n_1)$ in Equation \@ref(eq:VarianceRegressionEstimatorTwoPhase) are finite population corrections (fpc). These fpc's account for the reduced variance due to sampling the finite population and subsampling the first-phase sample without replacement.
