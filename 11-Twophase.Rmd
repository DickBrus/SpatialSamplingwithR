...
The estimated population mean equals `r round(mz,1)` g kg^-1^. The sampling variance over repeated sampling with both designs can be approximated^[In the approximation it is assumed that $N$ is much larger than $n_1$, and $(n_{1h}-1)/(n_1-1)$ is replaced by $n_{1h}/n_1$.] by (@sar92, Equation at bottom of p. 353)
...
The estimated standard error equals `r round(se_mz,1)` g kg^-1^.

The mean and its standard error can be estimated with functions `twophase` and `svymean` of package **survey** [@Lumley2020]. A data frame with the first-phase sample is assigned to argument `data` of function `twophase`. A column in this data frame, passed to argument `subset`, is an indicator with value `TRUE` if this unit is selected in the second phase, and `FALSE` otherwise. 
...
The estimated standard error equals `r round(se_mz_reg2ph,2)` 10^9^ kg ha^-1^.
...
1. Write an **R** script to select a simple random sample without replacement of 250 units from Eastern Amazonia (`data/grdAmazonia.rds`) and a subsample of 100 units by simple random sampling without replacement. Repeat this 1,000 times in a for-loop. 
