...
The estimated mean equals `r round(mz,1)` g kg^-1^, and the estimated standard error equals `r round(se_mz,1)` g kg^-1^. The sampling design is self-weighting, and so the estimated mean is equal to the sample mean.

```{r}
print(mean(mysample$z))
```

The same estimate is obtained with functions `svydesign` and `svymean` of package **survey** [@Lumley2020]. The estimator of the population total can be written as a weighted sum of the observations with all weights equal to $M/(n\;m)$. These weights are passed to argument `weight`. 
...
The variance of the 10,000 means with two-stage cluster random sampling equals `r as.character(round(var(mz),1))` (g kg^-1^)^2^. This is considerably larger than with simple random sampling: `r as.character(round(var(mz_SI),1))` (g kg^-1^)^2^. The average of the estimated variances with two-stage cluster random sampling equals `r as.character(round(mean(v_mz),1))` (g kg^-1^)^2^. 
...
The $\pi$ estimate equals 79.0, and the ratio estimate equals `r round(mz_ratio,1)` g kg^-1^. The $\pi$ estimate of the population mean can also be computed by first estimating totals of PSUs, see Equation \@ref(eq:EstTotalTwostageEqual). 
