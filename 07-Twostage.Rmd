...
twostage <- function(sframe, psu, n, m) {
  ...
  mypsusample <- sframe$psu[units]
  ssunits <- NULL
  for (i in seq_len(length(mypsusample))) { 
    ssunit <- sample(
      x = which(sframe$psu == mypsusample[i]), size = m, replace = TRUE)
...
design_2stage <- svydesign(
  id = ~ psudraw, weight = ~ weights, data = mysample)
...
n <- 6; m <- 10
pi <- inclusionprobabilities(M_psu, n)
set.seed(314)
...
design_str2stage <- svydesign(id = ~ psudraw, strata = ~ zone_stratum,
