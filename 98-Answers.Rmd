...
(ref:RegressionEstimatorsAmazoniaTwoPhaselabel) Approximated sampling distribution of the simple regression estimator of the mean AGB (10^9^ kg ha^-1^) in Eastern Amazonia in the case that the covariate is observed for all sampling units (One-phase) and for a subsample only (Two-phase).
...
```{r QuantilesOKVarGridspacing, echo=FALSE, fig.asp=0.7, fig.cap="Three quantiles of the ordinary kriging variance of predicted SOM concentrations in three woredas in Ethiopia, as a function of the grid spacing."}
...
```{r MBSupSamples, echo=FALSE, out.width='100%', fig.cap="Model-based sample for estimating the semivariogram, using the log of the determinant of the inverse Fisher information matrix (logdet) and the variance of the kriging variance (VKV) as a minimisation criterion. The sampling pattern is optimised with an exponential semivariogram with a range of 200 m and a ratio of spatial dependence of 0.5."}
